...
def build_people_and_teams(leaderboards: list[dict[str, Any]]) -> tuple[pd.DataFrame, pd.DataFrame]:
    all_df = pd.DataFrame.from_records(leaderboards)
    if all_df.empty:
        return pd.DataFrame(columns=["id", "name"]), pd.DataFrame(columns=["id", "first_name", "last_name"])

    teams_df = all_df[all_df["type"] == "team"][["id", "name"]].drop_duplicates().reset_index(drop=True)
    people_df = (
...
def parse_dailylog_edges(user_id: str, edges: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for edge in edges:
        ...
        if node_type == "day_entry":
            ...
            records.append({"user_id": user_id, "date": current_date, "step_count": step_count, "points": points})
    return records
...
def attach_team_membership(people_df: pd.DataFrame, memberships_df: pd.DataFrame) -> pd.DataFrame:
...
def build_cumulative_points(step_counts_df: pd.DataFrame) -> pd.DataFrame:
    ...
    cumulative = (
        ...
        .cumsum()
        .reset_index()
        .rename(columns={"step_count": "step_count_cumulative", "points": "points_cumulative"})
    )
    ranked = step_counts_df.merge(cumulative, on=["date", "user_id"], how="left")
    ranked["rank"] = ranked.groupby("date")["points_cumulative"].rank(method="dense", ascending=False)
    return ranked


def build_date_columns(year: int, month: int, end_date: date | None = None) -> list[date]:
...
def build_wide_export(
    ...
    base_columns = ["EntityType", "EntityName", "PNG", "Gender", "Height"]

    if selection_df.empty:
        return pd.DataFrame(columns=base_columns + date_columns)

    selected_rows = selection_df[selection_df["entity_type"].isin(["person", "team"])].copy()
    selected_rows = selected_rows.drop_duplicates(subset=["entity_type", "entity_id"]).reset_index(drop=True)
    ...
    selected_people = selected_rows[selected_rows["entity_type"] == "person"][["entity_id", "entity_name", "png", "gender", "height"]].copy()
    selected_teams = selected_rows[selected_rows["entity_type"] == "team"][["entity_id", "entity_name", "png", "gender", "height"]].copy()
    ...
    for user_id in person_ids:
        ...
        daily = filtered_steps[filtered_steps["user_id"] == str(user_id)].groupby("date", as_index=False)["points"].sum().sort_values("date")
    ...
    person_panel = pd.concat(person_frames, ignore_index=True) if person_frames else pd.DataFrame(columns=["user_id", "date", "value"])

    rows: list[dict[str, object]] = []
    for _, row in selected_people.iterrows():
        values = person_panel[person_panel["user_id"] == str(row["entity_id"])]
        export_row = {"EntityType": "person", "EntityName": row["entity_name"], "PNG": row["png"], "Gender": row["gender"], "Height": row["height"]}
    ...
    for _, row in selected_teams.iterrows():
        member_ids: list[str] = []
        if not memberships_df.empty:
            member_ids = memberships_df[memberships_df["team_id"].astype(str) == str(row["entity_id"])]["id"].dropna().astype(str).unique().tolist()
        ...
        if member_ids:
            ...
            if not team_panel.empty:
                ...
                for _, value_row in summed.iterrows():
                    team_values[format_export_date(value_row["date"])] = int(value_row["value"])
        export_row = {"EntityType": "team", "EntityName": row["entity_name"], "PNG": row["png"], "Gender": row["gender"], "Height": row["height"]}
...
def get_export_dates_with_data(step_counts_df: pd.DataFrame, year: int, month: int) -> list[date]:
    if step_counts_df.empty:
        return []
